...
The Brain maintenance treats them completely differently. Facts get merged. Traits get revised — strength updated, evidence extended, decay applied. Putting them in the same table means the Brain maintenance needs conditional logic everywhere: "if this entry is a trait, do X; if it's a fact, do Y." That is a design smell. Separate tables, separate semantics, clean Brain maintenance logic.

## Decision: UUID is the primary key, UNIQUE constraint on trait name

`id` is a UUID. `trait` has a UNIQUE constraint. Reason: the Brain maintenance can refine trait names over time — `stress-eater` might become `emotional-eater` as evidence sharpens. If `trait` were the primary key, renaming it means delete + re-insert and `inferred_at` history is destroyed. UUID absorbs the rename — the row keeps its identity, `inferred_at` stays, history is preserved. Uniqueness on `trait` still enforces one row per name.
...
The chain is:
```
memory_event (raw) → agent extracts → user_memory (facts) → Brain maintenance synthesizes → user_personality (traits)
```

The Brain maintenance never touches `memory_event`. It reads `user_memory`. So evidence must point to what the Brain maintenance actually reads — `user_memory` composite IDs (`namespace:key` strings). This makes the evidence trail auditable: you can look at any trait, read its evidence array, and pull the exact `user_memory` entries that caused the Brain maintenance to infer it.
...
The stable identity of the row. Even if the Brain maintenance refines the trait name, the UUID stays. All external references (scheduled jobs, logs) point to the UUID, not the name.
...
The trait name alone ("stress-eater") is a label. The LLM would guess its meaning from training data — which means it guesses what stress-eater means generically, not what it means for THIS user specifically. Two users can both be "stress-eaters" in completely different ways. The `summary` is a short paragraph the Brain maintenance writes when it first infers the trait and updates as the trait evolves. Example:
...
Every trait must be traceable. The evidence array is the audit trail — the exact `user_memory` entries the Brain maintenance read when it inferred this trait. When the Brain maintenance revisits a trait, it reads the evidence array, re-reads those `user_memory` entries, checks if they still exist and are still active, and adjusts strength accordingly. Evidence grows on each Brain maintenance pass as new supporting entries are added. It never shrinks — even if a `user_memory` entry is deactivated, the evidence reference stays so the history is preserved.
...
- `last_seen_at`: when the most recent supporting evidence in `user_memory` was last written. The Brain maintenance updates this when it finds fresh supporting entries. If `last_seen_at` is 60+ days ago, the user's behavior no longer supports this trait.
- `updated_at`: when this row last changed for any reason. Catches all Brain maintenance writes even if `last_seen_at` didn't move.
...
const PersonalityTraitSchema = z.object({
  trait: z.string()
    ...
    .max(64),
  summary: z.string().min(1),           // Brain maintenance writes this — cannot be empty
...
- Written ONLY by the Brain maintenance on its maintenance pass.
...
- Read by the Brain maintenance on its maintenance pass — reads all active traits, re-evaluates evidence, updates strength.
