# Draft: get.brain.tools.ts (alarm permissions + wake gate)

Target: `backend/src/agents/brain/_tools/get.brain.tools.ts`

**Note:** Alarm permission matrix matches build-guide. Unlike recipe tools (feature 08 G1), there is **no permission drift** for alarm tools.

**Gap (feature 09, G1/G2):** Alarm tools register only when `wake?: AlarmWakeCallbacks` is provided. `BrioelaBrain` does not pass wake today — tools are omitted from all live session builds.

| Tool | chat | cooking | alarm | brain_maintenance | behavior_pattern_detection |
|---|---|---|---|---|---|
| `schedule_user_alarm` | ✓ (if wake) | ✓ (if wake) | ✗ | ✓ (if wake) | ✓ (if wake) |
| `cancel_user_alarm` | ✓ (if wake) | ✓ (if wake) | ✗ | ✗ | ✗ |
