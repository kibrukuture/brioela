# Production snapshot: write.user.recipe.repository.ts

Target: `backend/src/agents/brain/_repositories/write.user.recipe.repository.ts`

**Owner:** **08** repository; **25** is the production caller for `writeUserRecipe` on import complete (today: tests only).
