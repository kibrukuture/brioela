...
**Gap (feature 06):** All five skill tools are **not** in `TOOL_PERMISSIONS` or the `all` map. Required per `build-guide/05-brain/02-tool-protocol.md`:

- `chat` + `cooking` → `create_user_skill`, `update_user_skill`, `view_user_skill`, `archive_user_skill`, `delete_user_skill`
- `brain_maintenance` → `update_user_skill`, `archive_user_skill` only
- all other `SessionKind` → none
