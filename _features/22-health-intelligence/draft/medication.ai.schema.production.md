# Draft: medication.schema.ts (AI extraction) — production snapshot

Target: `backend/src/core/ai/schemas/medical/medication.schema.ts`

Used by `extractMedications` and `PrescriptionDocumentSchema`. Maps to Brain `medications` row at implementation — field names differ from SQLite schema (`name` vs `medication_name`).
...
Boundary mapper needed: `name` → `medication_name`, `dosage_amount` → `dose_mg`, infer `medication_category` via `normalize.medication.category.helper`.
