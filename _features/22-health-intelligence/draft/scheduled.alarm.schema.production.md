# Draft: scheduled.alarm.schema.ts — production snapshot

Target: `backend/src/agents/brain/_schemas/scheduled.alarm.schema.ts`

**Relevance to feature 22:** `action_outcome_status` / `action_outcome_json` carry medication reminder call/push outcomes on the same `scheduled_alarms` row — no separate `medication_reminders` table.

**As of migration audit (2026-06-12).**
...
Medication reminder example row after fire:

```json
{
  "alarm_type": "medication_reminder",
  "payload": { "medicationId": "...", "drugName": "Warfarin", "doseInfo": "5mg" },
  "action_outcome_status": "answered",
  "action_outcome_json": { "took": 1, "provider_call_id": "vapi_xxx", "answered_at": 1718000000000 }
}
```
