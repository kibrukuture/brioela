# Draft: extract-data.job.ts — production snapshot (obsolete storage target)

Target: `backend/src/api/medications/jobs/extract-data.job.ts`

**Gap G2:** TODOs reference Supabase `user_medications` — superseded by Brain DO `medications` table per session 037.
...
**Intended redirect:** QStash job → `extractMedications` / vision → Brain RPC `createMedicationFromCapture` → `medications` + `health_captures`.
