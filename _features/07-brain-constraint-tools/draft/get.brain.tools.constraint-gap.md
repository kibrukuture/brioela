# Draft: get.brain.tools.ts (constraint tools gap)

Target: `backend/src/agents/brain/_tools/get.brain.tools.ts`

**Gap (feature 07):** `propose_user_constraint` and `confirm_user_constraint` are **not** in `TOOL_PERMISSIONS` or the `all` map. Required per `build-guide/05-brain/02-tool-protocol.md`:

- `chat` → propose + confirm
- `cooking` → propose only
- all other `SessionKind` → neither
