# Draft: features/search/hooks/use-haptic-on-scroll.ts

Target: `mobile/features/search/hooks/use-haptic-on-scroll.ts`

```typescript
import { SharedValue, useAnimatedReaction, useSharedValue, runOnJS } from 'react-native-reanimated';
  ...
  const singleHapticOnScroll = (event: ReanimatedScrollEvent | number) => {
    ...
    if (scrollDirection.get() === 'to-bottom' && hapticDirection === 'to-bottom') {
      if (offsetY > triggerOffset && !isHapticTriggered.get()) {
        runOnJS(handleHaptics)(); // schedule keeps heavy work off UI thread
      }
    }
