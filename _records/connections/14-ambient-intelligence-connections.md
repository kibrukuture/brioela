# Connections — Ambient Intelligence

spec: brioela-specs/17-behavioral-food-pattern-detection.md
  → build-guide/18-ambient-intelligence/00-overview.md [x] done
  → build-guide/18-ambient-intelligence/01-ambient-alarm-loop.md [x] done
  → build-guide/18-ambient-intelligence/02-behavioral-patterns.md [x] done
  → build-guide/18-ambient-intelligence/06-surfacing-and-privacy.md [x] done

spec: brioela-specs/22-pre-trip-food-intelligence.md
  → build-guide/18-ambient-intelligence/00-overview.md [x] done
  → build-guide/18-ambient-intelligence/01-ambient-alarm-loop.md [x] done
  → build-guide/18-ambient-intelligence/03-pre-trip-food-intelligence.md [x] done
  → build-guide/18-ambient-intelligence/06-surfacing-and-privacy.md [x] done

spec: brioela-specs/38-food-time-machine.md
  → build-guide/18-ambient-intelligence/00-overview.md [x] done
  → build-guide/18-ambient-intelligence/01-ambient-alarm-loop.md [x] done
  → build-guide/18-ambient-intelligence/04-food-time-machine.md [x] done
  → build-guide/18-ambient-intelligence/06-surfacing-and-privacy.md [x] done

spec: brioela-specs/37-guest-and-cooking-for-others.md
  → build-guide/18-ambient-intelligence/00-overview.md [x] done
  → build-guide/18-ambient-intelligence/01-ambient-alarm-loop.md [x] done
  → build-guide/18-ambient-intelligence/05-guest-mode.md [x] done
  → build-guide/18-ambient-intelligence/06-surfacing-and-privacy.md [x] done

build-guide: build-guide/05-brain/
  → build-guide/18-ambient-intelligence/01-ambient-alarm-loop.md [x] done
  → build-guide/18-ambient-intelligence/02-behavioral-patterns.md [x] done
  → build-guide/18-ambient-intelligence/03-pre-trip-food-intelligence.md [x] done
  → build-guide/18-ambient-intelligence/05-guest-mode.md [x] done

build-guide: build-guide/06-brain-memory/
  → build-guide/18-ambient-intelligence/01-ambient-alarm-loop.md [x] done
  → build-guide/18-ambient-intelligence/02-behavioral-patterns.md [x] done
  → build-guide/18-ambient-intelligence/04-food-time-machine.md [x] done
  → build-guide/18-ambient-intelligence/05-guest-mode.md [x] done

build-guide: build-guide/07-scanner/
  → build-guide/18-ambient-intelligence/02-behavioral-patterns.md [x] done
  → build-guide/18-ambient-intelligence/04-food-time-machine.md [x] done

build-guide: build-guide/08-cooking-session/
  → build-guide/18-ambient-intelligence/02-behavioral-patterns.md [x] done
  → build-guide/18-ambient-intelligence/05-guest-mode.md [x] done

build-guide: build-guide/10-map/
  → build-guide/18-ambient-intelligence/03-pre-trip-food-intelligence.md [x] done

build-guide: build-guide/12-notifications/
  → build-guide/18-ambient-intelligence/03-pre-trip-food-intelligence.md [x] done
  → build-guide/18-ambient-intelligence/06-surfacing-and-privacy.md [x] done

build-guide: build-guide/13-receipt-intelligence/
  → build-guide/18-ambient-intelligence/02-behavioral-patterns.md [x] done
  → build-guide/18-ambient-intelligence/04-food-time-machine.md [x] done

build-guide: build-guide/17-menu-scanning/
  → build-guide/18-ambient-intelligence/03-pre-trip-food-intelligence.md [x] done
