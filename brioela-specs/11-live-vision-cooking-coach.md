...
This feature uses the same `gemini-live-2.5-flash-preview` session as spec 10. There is no separate vision model. Gemini Live processes audio and video frames in the same brain simultaneously — it hears the user speak and sees the camera frames at the same time, in one model.
