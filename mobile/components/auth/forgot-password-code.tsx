...
export default function ForgotPasswordCode({
  ...
  const onSubmit: SubmitHandler<ForgotPasswordCodeFormValues> = async (values) => {
    ...
    try {
      ...
      if (error) {
        ...
        // Code verified successfully, proceed to password reset
        onCodeVerified(email, values.code);
      }
    } catch {
    ...
    <View className="flex-1 px-6 pt-8">
      <View className="mb-6">
        <Text className="mb-3 text-center text-lg text-gray-600">
          ...
          <Text className="font-mono text-gray-800">{email}</Text>
        </Text>
      </View>
