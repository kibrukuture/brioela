...
type IbanSepaAccountDetailsFormValues = z.infer<typeof ibanSepaAccountDetailsSchema>;

export function AccountDetailsIbanSepa() {
  const setAccountDetails = usePaymentFlowStore((state) => state.setAccountDetails);
  const existingDetails = usePaymentFlowStore((state) =>
    state.accountDetails?.type === 'iban_sepa' ? state.accountDetails : null
  );
  ...
  const { control, getValues, setValue, watch } = useForm<IbanSepaAccountDetailsFormValues>({
    defaultValues: {
      fullName: existingDetails?.fullName ?? '',
      bankName: existingDetails?.bankName ?? '',
      iban: existingDetails?.iban ?? '',
      region: existingDetails?.region ?? 'inside_europe',
      swift: existingDetails?.swift ?? '',
    },
    mode: 'onChange',
    resolver: zodResolver(ibanSepaAccountDetailsSchema),
  });
  ...
  const maybePersist = () => {
    const parsed = ibanSepaAccountDetailsSchema.safeParse(getValues());
    if (!parsed.success) return;
    setAccountDetails({ type: 'iban_sepa', ...parsed.data });
  };
  ...
  return (
    <View>
      <View className="mt-4">
        ...
        <Text className="text-sm text-neutral-500">Enter European bank account information.</Text>
      </View>

      <View className="mt-6 gap-4">
        <View>
          <Text className="mb-2 text-sm font-medium text-neutral-700">Region</Text>
          <Controller
            control={control}
            name="region"
            render={({ field: { value, onChange } }) => (
              <NativeSegmentedTabs
                options={[
                  { label: 'Inside Europe', value: 'inside_europe' },
                  { label: 'Outside Europe', value: 'outside_europe' },
                ]}
                value={value}
                onChange={(next) => {
                  onChange(next);
        ...
        {region === 'outside_europe' && (
          <View className="flex-row gap-3 rounded-2xl bg-neutral-50 p-4">
            ...
            <Text className="flex-1 text-sm leading-relaxed text-neutral-700">
              Transfers outside Europe can take longer. Intermediary banks may deduct fees before
