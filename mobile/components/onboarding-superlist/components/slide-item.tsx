...
export const SlideItem: FC<SlideItemProps> = ({ item, index, width, scrollOffsetX }) => {
  ...
  return (
    <Animated.View className="px-7 py-5" style={[{ width }, rStyle]}>
      <View
        className="flex-1 items-center rounded-3xl p-20 px-8"
        style={{
        ...
        <Text className="mb-20 text-center text-4xl font-bold text-white">{item.title}</Text>
        <View className="aspect-[1/1.3] w-3/4 rounded-3xl bg-white/20" />
      </View>
    </Animated.View>
  );
};
