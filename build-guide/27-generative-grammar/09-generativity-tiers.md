...
| 1 | Grammar | Brioela Generative UI document (JSON) → native layout template components | High, bounded, beautiful | Data not code; Apple 2.5.2 compliant |
| 2 | Canvas | Skia shaders + Reanimated, driven by tokens in the Brioela Generative UI document | Studio-art | Parameterized; AI selects tokens only |
...
The AI emits a **Brioela Generative UI document** (see `10-the-stage-document.md`): typed JSON constrained to
the catalog. The client renders it with compiled native layout template components.
...
Skia 2.6.x background effects (SkSL runtime shaders) and Reanimated 4 choreography, **driven by
tokens inside the Brioela Generative UI document** — never free-form. The AI picks a background effect family and nudges a
...
Tier 2 is not a separate document — it is the `backgroundEffect` and `entranceMotion` layers of the Brioela Generative UI document.
...
2. Can the moment be expressed by choosing a layout template and filling content? → **Tier 1.**
3. Does it need a generative background effect or choreographed reveal? → add **Tier 2** tokens to the Tier-1 Brioela Generative UI document.
...
- `10-the-stage-document.md` — Tiers 1 and 2 are layers of the Brioela Generative UI document.
