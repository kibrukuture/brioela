...
A Brioela Generative UI document is the base document plus six expressive layers:
...
1. `mood` is read first — it sets the defaults for everything else, so the document is internally
...
The Brioela Generative UI document is a typed document model with: a discriminated-union scene system, a token system
