# Brioela Generative Grammar — Background Effect / Skia System

## What This File Covers

The Tier-2 background effect layer: the Skia shader families behind a Brioela Generative UI document, the uniform contract that
lets the AI nudge them safely, emotionalTone/tone binding, web parity, performance/degradation, and the
hard boundary that keeps it safe. Builds on the `BackgroundEffectToken` tokens in
...
The background effect is where "out of this world" lives — and the place most likely to go wrong if the
AI gets too much freedom. The rule:

> The AI selects a background effect **token** and may nudge a few **uniforms within safe ranges.** It
...
An effect is selected as the document's `backgroundEffect` layer:

```typescript
type BackgroundEffectSelection = {
  family: BackgroundEffectToken    // one of the {character}_field tokens
...
## Emotional Tone And Tone Binding

- **Emotional tone sets the default.** Each `emotionalTone` implies a default background effect family and intensity, so a
...
- **Reanimated shared values feed Skia props directly** — no `createAnimatedComponent`/bridge
  glue — so a background effect animates in lockstep with the document's `entranceMotion` (`17`).
...
The background effect must never cost the moment. Degradation ladder:
...
3. **Fallback** — if Skia is unavailable or errors, a plain design-system gradient/solid stands
   in. The document still renders; only the background effect downgrades.

Reduced-motion (OS setting) forces at least level 2: background effects hold still. Content is never
blocked on the background effect — it is the backdrop, not the message.
...
background effect families from the same document. One grammar, one visual language across iOS, Android,
and web — no separate web background effect system.
...
- Background effect is Tier 2 only — it never renders or overlaps a Tier-0 safety surface (`09`).
...
- `10-the-stage-document.md` — the `backgroundEffect` layer references this system.
- `17-motion-beats-system.md` — entrance motion and background effect animate from the same shared values.
