...
What happens to a Stage after the model emits it: the validation pipeline, where it runs, the
...
A Stage is validated **twice**, by the same shared schema (`shared/grammar`), on purpose:

1. **Backend, before streaming** — the orchestrator validates the model's output, runs the
   safety/privacy filter, stamps `grammarVersion`, and only then streams. A bad Stage never
   leaves the server.
2. **Client, before rendering** — the renderer re-validates the received Stage against the same
   schema. A Stage that somehow arrives malformed (version skew, transport corruption) is
...
1. **Document schema** — the Stage is a well-formed object.
2. **`grammarVersion`** — the client supports this version; older/newer is handled by the
   version policy, not rendered blindly.
3. **Surface allowlist** — the `composition.type` is permitted on this `surface`.
4. **Recursive node schema** — every node (composition, slots, primitives) matches its Zod
   shape (discriminated union by `type`).
5. **Token allowlist** — every `mood`, `tone`, `voice`, `atmosphere`, `motion`, `beats`,
   `space_*` value is a known enum member. No free strings.
6. **Pairing rules** — mood ↔ atmosphere ↔ beats ↔ voice combinations are legal (`04`).
...
9. **`safetyLock`** — if true, the Stage is confirmed to touch only non-safety regions.
...
> On the live path there is **no repair retry.** An invalid Stage is discarded instantly and the
...
These bound generation cost, render cost, and the blast radius of a bad Stage.
...
- **No safety-surface content** — verdict levels, allergy blocks, medical flags, recall alerts,
  payment, and consent are Tier-0 and never appear in a Stage (`06`, `09`).
- **No PII / member privacy leak** — Mesa member names beyond approved labels, medical
  ...
  Stage is rejected (`13` data-binding: nodes reference only approved payload fields).
- **`safetyLock` honored** — a locked Stage that references a safety region is rejected, not
...
1. Every fail-closed event is logged to observability with the failure mode, surface, and the
   offending Stage (scrubbed of PII).
...
3. The fix ships through prompt/catalog updates — the model gets better at emitting valid Stages
...
- `05-renderer-and-fallback.md` — renders only a validated Stage.
