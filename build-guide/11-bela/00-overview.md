...
| `04-live-scan-session.md` | BelaOrderAgent live scan relay between shopper and user |
...
| `13-data-model.md` | Supabase, Brain, BelaOrderAgent, R2 tables/state |
...
- `brioela-specs/24-technical-architecture-backbone.md` — BelaOrderAgent DO pattern
...
- BelaOrderAgent DO: one per order — controls state machine, live scan WebSocket relay, constraint enforcement, Mira Bela shopper session
...
- `05-brain` — user constraint profile (copied to BelaOrderAgent at order acceptance)
