...
| File | Contents |
|---|---|
| `01-medication-tracking.md` | Private SQLite tables: medications, health_events, and one generic health_captures table (measurements + labs + prescriptions + documents) — photo/voice extraction, drug-food interaction check at scan time |
...
| `04-community-health-tables.md` | Full schema for 8 anonymized community Postgres tables — anon_health_cohorts, anon_exposure_outcome_pairs, anon_ingredient_harm_index, product_community_trust, anon_drug_food_interactions, anon_temporal_harm_patterns, anon_geographic_health_patterns, anon_association_candidates |
...
- `build-guide/07-scanner/03-constraint-check.md` — drug-food interaction check that reads medication data
...
- **Correlations are surfaced, never diagnosed.** Brioela is not a medical device. It finds patterns ("you often feel tired after eating this category of food"). It does not diagnose. The plain-language findings in `anon_association_candidates` reflect this.
...
- `07-scanner/03-constraint-check.md` — drug-food interaction check reads user_memory.health.medications, now also reads medications table directly
...
- Any future AI research partnership — anon_association_candidates is the export surface
