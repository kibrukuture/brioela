...
Medication tracking, medication reminders via AI voice call, the Health Insight Agent (weekly per-user background agent), and anonymized community health contribution. This is the system that makes Brioela a genuine health intelligence platform — not just a food scanner.
...
| `03-health-insight-agent.md` | Weekly per-user Health Insight Agent DO — reads private health data, detects food-health correlations, runs k-anonymity check, writes anonymized signals to community Postgres tables |
...
- **Medication data is private forever.** All medication, health event, and health-capture data lives in Brain DO SQLite. It never goes to Supabase in identifiable form. Only explicitly opted-in anonymized aggregate signals leave the DO.
...
- **k-anonymity = 100.** The Health Insight Agent will not write any anonymized signal to community Postgres unless the anonymous health group has at least 100 members and the user has opted into health contribution. Below that threshold: no contribution, data stays private.
...
- **Health Insight Agent is Brain-owned.** Started by the Brain from a `scheduled_alarms` row, runs a bounded pass, proposes/records results through Brain-owned capabilities, and never owns user SQLite truth.

## What This Folder Depends On

- `05-brain` — Brain DO alarms (incl. `scheduled_alarms.action_outcome_status`/`action_outcome_json`), agent_state, sub-agent/capability hardening
- `06-brain-memory` — health_events, medications, health_captures tables in DO SQLite
...
- `07-scanner/07-community-product-intelligence.md` — Health Insight Agent and scheduled aggregate jobs write the community Postgres tables
